...
class FreBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        lambda_res = torch.sigmoid(self.lambda_res)
...
class Bottleneck(nn.Module):
    def __init__(self, channels):
        ...
        # self.ca = ChannelAttention(channels*3, returnw=True)
        # self.sa = SpatialAttention(channels, returnw=True)
    ...
    def forward(self, x):
        ...
        w = w.view(-1, 3, self.channels)[:, :, :, None, None]
...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, base_dim=64):
        ...
        self.down1 = Down(base_dim)
        self.block2 = ResBlock(base_dim)
        self.down2 = Down(base_dim)
...
class Decoder(nn.Module):
    def __init__(self, base_dim=64, out_channels=3):
        ...
        self.up2 = Up(base_dim)
        self.block2 = ResBlock(base_dim)

        self.up1 = Up(base_dim)
        self.block1 = ResBlock(base_dim)
